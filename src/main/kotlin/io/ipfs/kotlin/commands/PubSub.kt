...
class PubSub(val ipfs: IPFSConnection) {
    ...
    fun pub(topic: String, message: String): String {

        val request = Request.Builder()
                .url("${ipfs.base_url}pubsub/pub?arg=$topic&arg=$message")
                .build()
    ...
    @JvmOverloads fun sub(topic: String, discover: Boolean = false): String {

        val request = Request.Builder()
                .url("${ipfs.base_url}pubsub/sub?arg=$topic&arg=$discover")
                .build()

        val response = ipfs.okHttpClient.newCall(request).execute().body()

        response.use {
            return it!!.charStream().readLines().map { adapter.fromJson(it) }.toString()
        }
